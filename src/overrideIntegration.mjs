import fs from "fs";
import path from "path";


const LOCAL_COMPONENT_DIR = "src/components";

export default function overrideIntegration() {
  return {
    name: "overrides",
    hooks: {
      "astro:config:setup"({ updateConfig, config }) {
        const components = fs.readdirSync("./" + LOCAL_COMPONENT_DIR).filter(x => x.endsWith(".astro"));

        updateConfig({
          vite: {
            plugins: [{
              enforce: "pre",
              name: "override",
              async resolveId(source, importer) {
                // 跳过样式文件的解析
                if (source.endsWith('.css') || source.endsWith('.scss') || source.endsWith('.less') || source.endsWith('.sass')) {
                  return null;
                }
                
                // 跳过非组件文件的解析
                if (!source.endsWith('.astro')) {
                  return null;
                }
                
                for (const component of components) {
                  // 只拦截完全匹配的组件名称
                  if (source === component && !source.includes(LOCAL_COMPONENT_DIR) && !source.includes("node_modules")) {
                    if (importer && importer.includes(LOCAL_COMPONENT_DIR)) {
                      return path.resolve("./node_modules/@astrojs/starlight/components/" + component);
                    } else {
                      return path.resolve("./" + LOCAL_COMPONENT_DIR + "/" + component);
                    }
                  }
                }
              }
            }]
          }
        })
      }
    }
  }
}
